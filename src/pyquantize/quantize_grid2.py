...
def quantize_grid(
	...
	tie: {'toward', 'away', 'even', 'odd', 'alternate', 'random', 'stochastic'}, default = 'even'
	...
	tie = mode if tie is None else tie

	if tie not in ('toward', 'away', 'even', 'odd', 'alternate', 'random'):
		raise ValueError("invalid tie. must be one of {'toward', 'away', 'even', 'odd', 'alternate', 'random'}")
	...
	if frac != tie_point:
			...
			raise RuntimeError("this branch should not be possible. there is a huge leap of faith by the programmer. please let them know!")
